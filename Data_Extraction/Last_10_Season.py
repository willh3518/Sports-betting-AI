...
output_csv = os.path.join(csv_folder, "last_10_games_avg.csv")

# 📌 StatMuse Query Template
STATMUSE_LAST_5_TEMPLATE = "https://www.statmuse.com/nba/ask/{player}-in-the-last-10-games"

def extract_last_10_avg(html):
...
async def scrape_last_10_avg(player_name, session):
    ...
    return extract_last_10_avg(html) if html else {"GP": "N/A"}  # Handle missing data

async def scrape_all_last_10_avg():
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [scrape_last_10_avg(player, session) for player in unique_players]
    ...
    logging.info(f"✅ Last 10 games average stats saved to '{output_csv}'")

if __name__ == "__main__":
    asyncio.run(scrape_all_last_10_avg())
