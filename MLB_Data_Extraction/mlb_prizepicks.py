...
"""
mlb_prizepicks.py
Fetch live MLB prop lines from PrizePicks and write them to CSV.
"""
...
CSV_FILENAME = os.path.join("..", "MLB_Prop_Data_CSV", "mlb_prizepicks.csv")
...
def main():
    ...
    # ── SPECIAL CASE: Shohei Ohtani also plays DH ──────────────────────────────
    mask = df["Player"] == "Shohei Ohtani"
    if mask.any():
        # turn "P" into "P/DH"
        df.loc[mask, "Position"] = df.loc[mask, "Position"].astype(str) + "/DH"
